...
public class Simulator {
	...
	public static void run() {
		...
		for (int i = turns; i > 0; i--) {
			...
			for (int j = 0; j < fleet.size(); j++) {
				...
				fleet.get(j).updateConditions();

				if (fleet.get(j).getHeight() <= 0) {
					Tower.getTower().unregister(fleet.get(j));
					fleet.remove(fleet.get(j));
					j--;
				}
			}
		}
	...
	public static void parsing(String filename) {
		...
		} catch (IOException | MyIllegalArgumentException e) {
